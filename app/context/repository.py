...
from app.context.models import Conversation, ContextValue, ConversationContext
...
def get_or_create_context(
    ...
    scaduta), ne crea uno vuoto agganciato allo stesso conversation_id -
    precaricando nome/telefono se il cliente li ha già in anagrafica
    (da una prenotazione precedente), cosi' non glieli richiediamo due volte.
    """
    ...
    if context is None or expired:
        ...
        if customer.get("full_name"):
            context.customer.full_name = ContextValue(
                value=customer["full_name"], source="SYSTEM", confirmed=True
            )
        if customer.get("phone_number"):
            context.customer.phone = ContextValue(
                value=customer["phone_number"], source="SYSTEM", confirmed=True
            )
