...
def _resolve_tz(tz_name: str | None) -> ZoneInfo:
    try:
        return ZoneInfo(tz_name or "Europe/Rome")
    except Exception:
...
def list_upcoming_for_customer(
    ...
    "Futuro" tiene conto anche dell'ORARIO, non solo della data: un
    appuntamento di oggi già passato (es. sono le 15:00 e l'appuntamento
    era alle 9:00) non deve più contare come "prossimo" - altrimenti un
    cliente con un solo appuntamento reale ancora da venire si vede
    conteggiati due appuntamenti (quello di stamattina, già concluso, e
    quello vero). Il filtro lato query resta solo sulla DATA (>= oggi,
    più economico), il filtro sull'ORARIO di oggi si fa qui in Python.
    """
    sb = get_supabase()
    ...
    upcoming = [
        r for r in rows
        if r["appointment_date"] != today or str(r["appointment_time"]) >= now_time_str
    ]
